...
contract DeployPools is Script {
    ...
    function run() public {
        ...
        int24[] memory tickSpacings = abi.decode(jsonConstants.parseRaw(".tickSpacing"), (int24[]));
        ...
        for (uint256 i = 0; i < tokenAs.length; i++) {
            pool = v3Factory.getPool({tokenA: tokenAs[i], tokenB: tokenBs[i], tickSpacing: tickSpacings[i]});
